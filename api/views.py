...
from .serializers import LoginSerializer, PasswordResetConfirmSerializer, PasswordResetSerializer
        ...
        return Response(
            {
                ...
                "documentation": {"swagger": "/swagger/", "redoc": "/redoc/"},
            }
        )
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    {"status": "error", "message": "Failed to send reset email", "error": str(e)},
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if not uid or not token_from_url:
                return Response(
                    {"status": "error", "message": "Missing uid or token parameters"},
            ...
            if not default_token_generator.check_token(user, token_from_url):
                return Response(
                    {"status": "error", "message": "Invalid or expired token"}, status=status.HTTP_400_BAD_REQUEST
                )
