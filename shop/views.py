...
from .serializers import (
    ...
    CartAddSerializer, CartRemoveSerializer
)
...
class ProductListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return ProductInfo.objects.select_related('product', 'shop').filter(
            shop__state=True,
            quantity__gt=0
        ).distinct()
...
class ShopToggleView(APIView):
    ...
    def post(self, request, pk):
        ...
        return Response({
            'status': 'success',
            'message': f'Shop state changed to {"active" if shop.state else "inactive"}'
        })
...
class CartAddView(APIView):
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'product_info_id': openapi.Schema(
                    ...
                    description='ID товара (ProductInfo)',
                    example=1
                ),
    ...
    def post(self, request):
        ...
        if product_info.quantity < quantity:
            return Response({
                'status': 'error',
                'message': f'Not enough quantity. Available: {product_info.quantity}'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'contact_id': openapi.Schema(
                    type=openapi.TYPE_INTEGER,
                    description='ID контакта (адреса доставки)',
                    example=1
                ),
            },
            required=['contact_id']
...
class PartnerImportView(APIView):
    ...
    def post(self, request):
        ...
        if request.user.type != 'shop':
            return Response({
                'status': 'error',
                'message': 'Only shops can import products'
            }, status=status.HTTP_403_FORBIDDEN)
        ...
        if 'shop' not in data or 'categories' not in data or 'goods' not in data:
            return Response({
                'status': 'error',
                'message': 'Invalid data format. Required fields: shop, categories, goods'
            }, status=status.HTTP_400_BAD_REQUEST)
...
class PartnerExportView(APIView):
    ...
    def get(self, request):
        ...
        if request.user.type != 'shop':
            return Response({
                'status': 'error',
                'message': 'Only shops can export products'
            }, status=status.HTTP_403_FORBIDDEN)
