...
from .serializers import CategorySerializer, ContactSerializer, ProductInfoSerializer, ProductSerializer, ShopSerializer
...
class ShopModelTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="shop@example.com", password="testpass123", type="shop")
        self.shop = Shop.objects.create(name="Test Shop", url="http://test.com", user=self.user, state=True)
    ...
    def test_shop_unique_user(self):
        ...
        with self.assertRaises(IntegrityError):
            Shop.objects.create(name="Another Shop", user=self.user)
...
class ProductModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name="iPhone 15", category=self.category, description="Latest iPhone model"
        )
...
class ProductInfoModelTest(TestCase):
    ...
    def test_unique_product_info(self):
        ...
        with self.assertRaises(IntegrityError):
            ProductInfo.objects.create(product=self.product, shop=self.shop, external_id=123, quantity=5, price=89900)
...
class ProductParameterModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(name="iPhone 15", category=self.category)
        self.product_info = ProductInfo.objects.create(
            product=self.product, shop=self.shop, external_id=123, quantity=10, price=99900
        )
        self.parameter = Parameter.objects.create(name="Color")
    ...
    def test_unique_product_parameter(self):
        ...
        with self.assertRaises(IntegrityError):
            ProductParameter.objects.create(product_info=self.product_info, parameter=self.parameter, value="White")
...
class OrderModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.order = Order.objects.create(user=self.user, state="basket", contact=self.contact)
        self.shop = Shop.objects.create(name="Test Shop")
        self.category = Category.objects.create(name="Electronics")
        self.product = Product.objects.create(name="iPhone 15", category=self.category)
        self.product_info = ProductInfo.objects.create(
...
class APITest(TestCase):
    ...
    def test_cart_add(self):
        ...
        response = self.client.post("/api/v1/cart/add/", {"product_info_id": self.product_info.id, "quantity": 2})
        self.assertEqual(response.status_code, 200)
    ...
    def test_cart_add_not_enough_quantity(self):
        ...
        response = self.client.post("/api/v1/cart/add/", {"product_info_id": self.product_info.id, "quantity": 100})
    ...
    def test_order_create(self):
        ...
        self.client.post("/api/v1/cart/add/", {"product_info_id": self.product_info.id, "quantity": 2})

        # Создаем заказ
        response = self.client.post("/api/v1/orders/create/", {"contact_id": self.contact.id})
    ...
    def test_order_create_invalid_contact(self):
        ...
        response = self.client.post("/api/v1/orders/create/", {"contact_id": 999})
    ...
    def test_orders_list(self):
        ...
        # Создаем заказ
        self.client.post("/api/v1/cart/add/", {"product_info_id": self.product_info.id, "quantity": 2})
        self.client.post("/api/v1/orders/create/", {"contact_id": self.contact.id})
